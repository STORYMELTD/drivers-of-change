...
**Reveal modes** (`?reveal=`, read once in `startJourney`): **`slide`** (default, production) — city held off-screen right and slides in 0.80→1.0 over the fading gate (`.mural-viewport` raised to z 30 via `applyRevealZ`); **`crossfade`** (escape hatch) — city held at x=0 behind the gate, revealed as the gate fades.
